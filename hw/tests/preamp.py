...
def print_led_state(led: int):
  ...
  if led is not None:
    ...
    print('  ------------------------------------')
    print(
        f'    {rg:^10} | {zones[0]} | {zones[1]} | {zones[2]} | {zones[3]} | {zones[4]} | {zones[5]}')
...
def print_status(p: rt._Preamps, u: int):
  ...
  print(
      f'  Version {major}.{minor}-{git_hash:07X}, {"dirty" if dirty else "clean"}')

  # Power
  pstat = p.read_power_status(u)
  print(
      f'  Power: PG_5VD={pstat.pg_5vd}, PG_5VA={pstat.pg_5va}, PG_9V={pstat.pg_9v}, PG_12V={pstat.pg_12v}')
  print(
      f'         9V_EN={pstat.en_9v}, 12V_EN={pstat.en_12v}, {pstat.v12} V')
  ...
  if ctrl == rt.FanCtrl.MAX6644:
    ...
    fan_str = 'On, '
  print(
      f'  Fans: {fan_str}Control={ctrl.name}, Overtemp={ovr_tmp}, SMBus={smbus}')
...
def self_check(p: rt._Preamps, unit: Optional[int] = None) -> bool:
  ...
  for u in range(len(p.preamps)):
    if not unit or u + 1 == unit:  # u is 0-based, unit is 1-based
      ...
      print_cond(u, pstat.pg_5vd,
                 f'PG_5VD {"ok" if pstat.pg_5vd else "bad"}')
      print_cond(u, pstat.pg_5va,
                 f'PG_5VA {"ok" if pstat.pg_5va else "bad"}')
      print_cond(u, pstat.pg_9v,
                 f'PG_9V {"ok" if pstat.pg_9v else "bad"}')
      print_cond(u, pstat.pg_12v,
                 f'PG_12V {"ok" if pstat.pg_12v else "bad"}')
      print_cond(
          u, v12_ok, f'12V supply {"ok" if v12_ok else "bad"} - {pstat.v12:.2f}V')
      ...
      print_cond(
          u, amp1_ok, f'AMP1 Temp {"ok" if amp1_ok else "bad"}  - {temp2str(amp1_tmp)}')
      print_cond(
          u, amp2_ok, f'AMP2 Temp {"ok" if amp2_ok else "bad"}  - {temp2str(amp2_tmp)}')
      print_cond(
          u, hv1_ok, f'HV1 Temp {"ok" if hv1_ok else "bad"}   - {temp2str(hv1_tmp)}')
      if hv2_present:
        print_cond(
            u, hv2_ok, f'HV2 Temp {"ok" if hv2_ok else "bad"}   - {temp2str(hv2_tmp)}')
      print()
  return success
...
if not args.bootload:
  if len(preamps.preamps) < args.unit:
    ...
    if not args.quiet:
      # Wait a bit to make sure internal I2C writes have finished
      time.sleep(0.1)
