...
def main() -> None:
    ...
    try:
        ...
        board_rev_input = input(
            "Enter streamer board revision (ie: 1a):").strip()
        board_rev_match = BOARD_REV_RE.fullmatch(board_rev_input)
        if board_rev_match and len(board_rev_match.groups()) == 2:
            board_rev = (int(board_rev_match.group(1)),
                         board_rev_match.group(2))
        else:
    ...
    try:
        print(
            f"Writing streamer config to eeprom: serial={serial}, board_rev={board_rev[0]}{board_rev[1]}")
        eeprom = EEPROM(BoardType.STREAMER_SUPPORT)

        br = BoardRev(board_rev[0], board_rev[1])
        if serial == -1:
            eeprom.erase()
        else:
            eeprom.write_board_info(BoardInfo(serial=serial, unit_type=UnitType.STREAMER,
                                    board_type=BoardType.STREAMER_SUPPORT, board_rev=br))
    ...
    try:
        ...
        response = requests.post(
            'http://0.0.0.0/api/factory_reset', timeout=10)
        if not response.ok:
