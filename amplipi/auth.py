...
def _get_users() -> dict:
  ...
  try:
    ...
    logger.exception(f'Error loading users file: {e}')
    logger.info('Creating a users file from defaults.')
    ...
    logger.exception(f'Error loading users file as JSON: {e}')
    logger.warning('Moving the old one to a backup and creating a users file from defaults.')
...
def _authenticate_user_with_password(username: str, password: str) -> bool:
  ...
  try:
    ...
    logger.exception(f"exception in _verify_password(): {e}")
    return False
