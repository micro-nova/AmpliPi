...
# we don't import ctrl here to avoid naming ambiguity with a ctrl variable
from amplipi.ctrl import Api, ApiResponse, ApiCode, RCAs
...
class WebAPI:
  ...
  def __init__(self, settings: models.AppSettings):
    self.ctrl = Api(settings)
    # we host docs using rapidoc instead via a custom endpoint, so the default endpoints need to be disabled
    self.app = FastAPI(openapi_url=None, redoc_url=None)
    self.api = SimplifyingRouter()
    self.subscribers: Dict[int, 'Queue[models.Status]'] = {}
    ...
    self.app.mount("/generated", StaticFiles(directory=GENERATED_DIR), name="generated") # TODO: make this register as a dynamic folder???

    @self.api.get('/api', tags=['status'])
    def get_status() -> models.Status:
      """ Get the system status and configuration """
      return self.ctrl.get_state()

    @self.api.post('/api/load', tags=['status'])
    def load_config(config: models.Status) -> models.Status:
      ...
      self.ctrl.reinit(settings=self.ctrl._settings, change_notifier=self.notify_on_change, config=config)
      return self.ctrl.get_state()

    @self.api.post('/api/factory_reset', tags=['status'])
    def load_factory_config() -> models.Status:
      ...
      """
      self.ctrl.reinit(settings=self.ctrl._settings, change_notifier=self.notify_on_change, config=models.Status(**self.ctrl.DEFAULT_CONFIG))
      return self.ctrl.get_state()

    @self.api.post('/api/reset', tags=['status'])
    def reset() -> models.Status:
      """ Reload the current configuration, resetting the firmware in the process. """
      self.ctrl.reinit(settings=self.ctrl._settings, change_notifier=self.notify_on_change)
      return self.ctrl.get_state()

    @self.api.post('/api/reboot', tags=['status'],
    ...
    @self.api.post('/api/shutdown', tags=['status'],
      response_class=Response,
      responses = {
          200: {}
      }
    ...
    # sources
    @self.api.get('/api/sources', tags=['source'])
    def get_sources() -> Dict[str, List[models.Source]]:
      """ Get all sources """
      return {'sources' : self.ctrl.get_state().sources}

    @self.api.get('/api/sources/{sid}', tags=['source'])
    def get_source(sid: int = params.SourceID) -> models.Source:
      """ Get Source with id=**sid** """
      # TODO: add get_X capabilities to underlying API?
      sources = self.ctrl.get_state().sources
      return sources[sid]

    @self.api.patch('/api/sources/{sid}', tags=['source'])
    def set_source(update: models.SourceUpdate, sid: int = params.SourceID) -> models.Status:
    ...
    @self.api.get('/api/sources/{sid}/image/{height}', tags=['source'],
    ...
    async def get_image(sid: int = params.SourceID, height: int = params.ImageHeight):
      ...
      source_info = self.ctrl.status.sources[sid].info
    ...
    @self.api.get('/api/zones', tags=['zone'])
    def get_zones() -> Dict[str, List[models.Zone]]:
      """ Get all zones """
      return {'zones': self.ctrl.get_state().zones}

    @self.api.get('/api/zones/{zid}', tags=['zone'])
    def get_zone(zid: int = params.ZoneID) -> models.Zone:
    ...
    @self.api.patch('/api/zones/{zid}', tags=['zone'])
    def set_zone(zone: models.ZoneUpdate, zid: int = params.ZoneID) -> models.Status:
      """ Update a zone's configuration (zone=**zid**) """
      return self.code_response(self.ctrl.set_zone(zid, zone))

    @self.api.patch('/api/zones', tags=['zone'])
    def set_zones(multi_update: models.MultiZoneUpdate) -> models.Status:
      """ Update a bunch of zones (and groups) with the same configuration changes """
      return self.code_response(self.ctrl.set_zones(multi_update))

    # groups

    @self.api.post('/api/group', tags=['group'])
    ...
    @self.api.get('/api/groups', tags=['group'])
    def get_groups() -> Dict[str, List[models.Group]]:
      """ Get all groups """
      return {'groups' : self.ctrl.get_state().groups}

    @self.api.get('/api/groups/{gid}', tags=['group'])
    def get_group(gid: int = params.GroupID) -> models.Group:
    ...
    @self.api.patch('/api/groups/{gid}', tags=['group'])
    def set_group(group: models.GroupUpdate, gid: int = params.GroupID) -> models.Status:
      """ Update a groups's configuration (group=**gid**) """
      return self.code_response(self.ctrl.set_group(gid, group))

    @self.api.delete('/api/groups/{gid}', tags=['group'])
    def delete_group(gid: int = params.GroupID) -> models.Status:
      """ Delete a group (group=**gid**) """
      return self.code_response(self.ctrl.delete_group(gid))

    # streams

    @self.api.post('/api/stream', tags=['stream'])
    def create_stream(stream: models.Stream) -> models.Stream:
    ...
    @self.api.get('/api/streams', tags=['stream'])
    def get_streams() -> Dict[str, List[models.Stream]]:
      """ Get all streams """
      return {'streams' : self.ctrl.get_state().streams}

    @self.api.get('/api/streams/{sid}', tags=['stream'])
    def get_stream(sid: int = params.StreamID) -> models.Stream:
    ...
    @self.api.patch('/api/streams/{sid}', tags=['stream'])
    def set_stream(update: models.StreamUpdate, sid: int = params.StreamID) -> models.Status:
      """ Update a stream's configuration (stream=**sid**) """
      return self.code_response(self.ctrl.set_stream(sid, update))

    @self.api.delete('/api/streams/{sid}', tags=['stream'])
    def delete_stream(sid: int = params.StreamID) -> models.Status:
      """ Delete a stream """
      return self.code_response(self.ctrl.delete_stream(sid))

    @self.api.post('/api/streams/{sid}/station={station}', tags=['stream'])
    def change_station(sid: int = params.StreamID, station: int = params.StationID) -> models.Status:
    ...
    @self.api.post('/api/streams/{sid}/{cmd}', tags=['stream'])
    def exec_command(cmd: models.StreamCommand, sid: int = params.StreamID) -> models.Status:
    ...
    @self.api.post('/api/preset', tags=['preset'])
    def create_preset(preset: models.Preset) -> models.Preset:
      """ Create a new preset configuration """
      return self.code_response(self.ctrl.create_preset(preset))

    @self.api.get('/api/presets', tags=['preset'])
    def get_presets() -> Dict[str, List[models.Preset]]:
      """ Get all presets """
      return {'presets' : self.ctrl.get_state().presets}

    @self.api.get('/api/presets/{pid}', tags=['preset'])
    def get_preset(pid: int = params.PresetID) -> models.Preset:
    ...
    @self.api.patch('/api/presets/{pid}', tags=['preset'])
    def set_preset(update: models.PresetUpdate, pid: int = params.PresetID) -> models.Status:
      """ Update a preset's configuration (preset=**pid**) """
      return self.code_response(self.ctrl.set_preset(pid, update))

    @self.api.delete('/api/presets/{pid}', tags=['preset'])
    def delete_preset(pid: int = params.PresetID) -> models.Status:
      """ Delete a preset """
      return self.code_response(self.ctrl.delete_preset(pid))

    @self.api.post('/api/presets/{pid}/load', tags=['preset'])
    def load_preset(pid: int = params.PresetID) -> models.Status:
      """ Load a preset configuration """
      return self.code_response(self.ctrl.load_preset(pid))

    # PA

    @self.api.post('/api/announce', tags=['announce'])
    def announce(announcement: models.Announcement) -> models.Status:
      """ Make an announcement """
      return self.code_response(self.ctrl.announce(announcement))

    # Info

    @self.api.get('/api/info', tags=['status'])
    def get_info() -> models.Info:
      """ Get additional information """
      return self.code_response(self.ctrl.get_info())

    # @self.api.get('/api/subscribe') # TODO: uncomment this to add SSE Support and properly document it
    async def subscribe(req: Request):
      """ Subscribe to SSE events """
      msg_que: Queue = Queue(3)
      next_sub = max(self.subscribers.keys(), default=0) + 1
      self.subscribers[next_sub] = msg_que
      async def stream():
        try:
          while True:
    ...
    def read_openapi_yaml() -> Response:
      ...
      return Response(self.create_yaml_doc(), media_type='text/yaml')
    ...
    self.app.openapi = self.generate_openapi_spec # type: ignore

    # Documentation

    @self.app.get('/doc', include_in_schema=False)
    def doc():
    ...
    @self.app.get('/{src}', include_in_schema=False)
    def view(request: Request, src: int = 0):
      """ Webapp main view """
      ...
      context = {
        ...
        'inputs': [self.ctrl.get_inputs(src) for src in state.sources],
        'unused_groups': [unused_groups(self.ctrl, src.id) for src in state.sources if src.id is not None],
        'unused_zones': [unused_zones(self.ctrl, src.id) for src in state.sources if src.id is not None],
        'ungrouped_zones': [ungrouped_zones(self.ctrl, src.id) for src in state.sources if src.id is not None],
  ...
  def code_response(self, resp: Union[ApiResponse, models.BaseModel]):
  ...
  def notify_on_change(self, status: models.Status) -> None:
    """ Notify subscribers that something has changed """
    for msg_que in self.subscribers.values():
      msg_que.put(status)

  # API Documentation

  def get_body_model(self, route: APIRoute) -> Optional[Dict[str, Any]]:
  ...
  def get_response_model(self, route: APIRoute) -> Optional[Dict[str, Any]]:
  ...
  def add_creation_examples(self, openapi_schema, route: APIRoute) -> None:
    """ Add creation examples for a given route (for modifying request types) """
    req_model = self.get_body_model(route)
  ...
  def add_response_examples(self, openapi_schema, route: APIRoute) -> None:
    """ Add response examples for a given route """
    resp_model = self.get_response_model(route)
  ...
  def get_live_examples(self, tags: List[Union[str, Enum]]) -> Dict[str, Dict[str, Any]]:
    """ Create a list of examples using the live configuration """
    live_examples = {}
    for tag in tags:
      for i in self.ctrl.get_items(str(tag)) or []:
  ...
  def generate_openapi_spec(self, add_test_docs=True):
    ...
    for route in self.app.routes:
      if isinstance(route, APIRoute):
        self.add_creation_examples(openapi_schema, route)
        self.add_response_examples(openapi_schema, route)
    ...
    for route in self.app.routes:
      if isinstance(route, APIRoute):
        self.add_example_params(openapi_schema, route)

    return openapi_schema

  @lru_cache(2)
  def create_yaml_doc(self, add_test_docs=True) -> str:
...
def create_app(mock_ctrl=None, mock_streams=None, config_file=None, delay_saves=None, settings: models.AppSettings = models.AppSettings()) -> FastAPI:
  ...
  api = WebAPI(settings) # api gets hooked here TODO: add hook method to WebAPI?
  return api.app
...
if __name__ == '__main__':
  ...
  api = WebAPI(settings=models.AppSettings())
  with args.file as file:
    file.write(api.create_yaml_doc(add_test_docs=False))
