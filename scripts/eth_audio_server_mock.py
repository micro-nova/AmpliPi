...
status = { # This is the system state response that will come back from the ethaudio box
    "power": {
...
#}
def set_source(id, name, digital):
    idx = None
...
#}
def set_zone(id, name, source_id, mute, stby, vol, disabled):
    idx = None
    for i, s in enumerate(status['zones']):
        if s['id'] == id:
            idx = i
    if idx is not None:
        try:
            status['zones'][idx]['name'] = str(name)
            status['zones'][idx]['source_id'] = parse_int(source_id, [1, 2, 3, 4])
            status['zones'][idx]['mute'] = bool(mute)
...
def show_change():
    global last_status
    diff = deepdiff.DeepDiff(last_status, status)
...
if __name__ == "__main__":
    ...
    show_change()
    set_source(0, 'Spotify', True)
    show_change()
    set_source(1, 'Pandora', True)
    show_change()
    set_source(2, 'TV', False)
    show_change()
    set_source(3, 'PC', False)
    show_change()
    set_zone(0, 'Party Zone', 1, False, False, 0, False)
    show_change()
    set_zone(1, 'Drone Zone', 2, False, False, -20, False)
    show_change()
    set_zone(2, 'Sleep Zone', 3, True, False, -40, False)
    show_change()
    set_zone(3, "Standby Zone", 4, False, True, -50, False)
    show_change()
    set_zone(4, "Disabled Zone", 1, False, False, 0, True)
    show_change()

    # Test string/json based command handler
    for cmd in test_cmds:
        test_cmd(cmd) # TODO: add expected result checker here
        show_change()
