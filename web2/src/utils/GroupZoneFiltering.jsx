...
export const groupZoneMatchCount = (zones, group) =>
    zones.reduce((sum, zone) => sum + (group.zones.includes(zone.id) ? 1 : 0), 0);

const groupIsSubset = (group, zones) =>
    group.zones.every((zid) => zones.map((z) => z.id).includes(zid));
