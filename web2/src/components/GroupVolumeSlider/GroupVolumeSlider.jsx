...
const GroupVolumeSlider = ({ groupId }) => {
  const volume = useStatusStore(
    (s) => s.status.groups.filter((g) => g.id === groupId)[0].vol_f
  )
  const setGroupVol = useStatusStore((s) => s.setGroupVol)
  const setGroupMute = useStatusStore((s) => s.setGroupMute)
  const group = useStatusStore(
    (s) => s.status.groups.filter((g) => g.id === groupId)[0]
  )
  ...
  return (
    <div className="group-volume-container">
      ...
      {slidersOpen && (
        <div className="group-volume-children-container">{zones}</div>
      )}
    </div>
  )
}
