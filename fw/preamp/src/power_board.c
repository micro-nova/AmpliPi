...
void enablePowerBoard() {
  ...
  writeI2C2(pwr_temp_mntr_dir, 0x3C);  // 0=output, 1=input
}
...
int read_ADC() {
  // Taken from the latter half of readI2C2(). The ADC only has the one reg to
  // read from, so none of the reg specifying is necessary
  while (I2C_GetFlagStatus(I2C2, I2C_FLAG_BUSY)) {}
