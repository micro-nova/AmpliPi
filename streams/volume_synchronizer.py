...
class VolSyncDispatcher:
  ...
  # All you need to do to use this class is build a StreamWatcher extension and then follow the above example with a simple argparse flow, everything else is handled automatically
    ...
    while True:
      try:
        if self.vol_set_point is None:
          # While vol_set_point is set to amplipi vol by default, sometimes amplipi vol isn't read at initialization
          # In that case, set the set point to AmpliPi volume and then set remote vol to match
          self.vol_set_point = float(self.amplipi.volume)
          if self.amplipi.volume is not None:
            self.schedule_event(VolEvents.CHANGE_STREAM)

        event = self.event_queue.get()
        if event == VolEvents.CHANGE_AMPLIPI:
          self.logger.debug(f"Attempting to set AmpliPi volume to {self.stream.volume}")
          self.vol_set_point = self.amplipi.set_vol(self.stream.volume, self.vol_set_point)
        elif event == VolEvents.CHANGE_STREAM:
          self.logger.debug(f"Attempting to set remote volume to {self.amplipi.volume}")
